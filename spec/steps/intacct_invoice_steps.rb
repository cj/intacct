module IntacctInvoiceSteps
  ...
  def custom_fields_form_auto
    # xml.termname "#{intacct_customer_data[:termname].present? ? intacct_customer_data[:termname] : "Net 30"}"
    xml.customfields {
      xml.customfield {
      ...
      xml.customfield {
        ...
        xml.customfieldvalue object.invoice.mileage_miles.to_i
      }
      xml.customfield {
    ...
    xml.invoiceitems {
      ...
      xml.lineitem {
        xml.glaccountno gl_revenue_account_number(object.invoice.type)
        xml.amount standard_amount
        xml.memo object.invoice.note
        xml.locationid "ACDCorp"
        xml.customerid object.invoice.claim.company.intacct_system_id
      ...
      if object.invoice.mileage_fee!=0
        xml.lineitem {
          xml.glaccountno gl_revenue_account_number(:mileage)
